# KorrinOS v1.2 — THE REAL FULL DISTRIBUTION

Goal: a REAL, multi-GB bootable OS. NOT a 600MB stub.

The v1.1 mistake:
- Stock Ubuntu kernel in casper/vmlinuz (NOT our tree's KorrinOS kernel)
...
### 1. Kernel — KorrinOS's OWN, compiled from this tree
...
### 4. Baking
- `/opt/korrinos/os` + worlds + modes.sh + tinker-world launcher
- casper live layout + squashfs (xz or gzip for size/speed balance)
- grub-mkrescue ISO, KorrinOS-v1.2.iso (expect MULTI-GB)

## Verification
- file KorrinOS-v1.2.iso  -> ISO 9660, bootable
- unsquashfs -ll casper/filesystem.squashfs | grep -E opt/korrinos|usr/bin/nmap|usr/lib/steam
- bzImage inside IS 'KorrinOS' built tree kernel (128MB expected)
- du -sh -> GBs not MBs