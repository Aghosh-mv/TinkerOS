#!/usr/bin/env bash
# tinkeros-uninstall-blocker.sh — Prevents TinkerAI from being removed
# Runs as a cron job that checks and restores files

PROTECTED_DIR="/usr/local/tinkeros"
...
verify_integrity() {
  ...
  if [ "$bad" -gt 0 ]; then
    ...
    # In real TinkerOS, would restore from read-only image
    return 1
  fi
  return 0
}
...
if [ "${1:-}" = "--setup-cron" ]; then
  ...
  (crontab -l 2>/dev/null | grep -v "tinkeros-uninstall-blocker" ; echo "*/5 * * * * /usr/local/tinkeros/config/uninstall-blocker.sh --protect >> /var/log/tinkeros-protect.log 2>&1") | crontab -
