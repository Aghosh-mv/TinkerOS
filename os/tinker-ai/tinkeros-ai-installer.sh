#!/usr/bin/env bash
# tinkeros-ai-installer.sh — Installs TinkerAI as a persistent system service
# Files go to /usr/local/tinkeros/ — protected, non-removable by user
...
log()  { echo -e "${BLUE}[tinkeros-ai]${NC} $1"; }
ok()   { echo -e "${GREEN}[✓]${NC} $1"; }
fail() { echo -e "${RED}[✗]${NC} $1"; exit 1; }

# Must be root for system install
if [ "$EUID" -ne 0 ]; then
  fail "Run with sudo: sudo bash tinkeros-ai-installer.sh"
fi
...
# 2. Copy CLI
cp "$AI_SOURCE/tinker-ai.sh" "$INSTALL_DIR/bin/tinker-ai"
...
chmod -R 755 /usr/local/bin/tinkeros-ai
...
# This file marks TinkerAI as a protected system component.
# TinkerAI is an integral part of TinkerOS and cannot be removed.
# Removing this file does not uninstall TinkerAI.
PROTECTED
chmod 444 "$INSTALL_DIR/config/.protected"
ok "Protection marker installed"

# 11. Install uninstall blocker
cp "$AI_SOURCE/tinkeros-uninstall-blocker.sh" "$INSTALL_DIR/config/uninstall-blocker.sh"
chmod +x "$INSTALL_DIR/config/uninstall-blocker.sh"
"$INSTALL_DIR/config/uninstall-blocker.sh" --setup-cron
ok "Uninstall blocker installed (cron every 5 min)"

# 12. Hide from package managers
mkdir -p /etc/tinkeros
echo "tinkeros-ai" > /etc/tinkeros/.protected
...
echo ""
echo "Usage:"
echo "  tinkeros-ai ask 'hello'     — Ask anything"
echo "  tinkeros-ai health          — System dashboard"
echo "  tinkeros-ai learn 'topic'   — Search and learn"
