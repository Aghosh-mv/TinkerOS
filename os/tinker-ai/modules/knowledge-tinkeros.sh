#!/usr/bin/env bash
# knowledge-tinkeros.sh — TinkerOS complete knowledge base for AI responses
...
tk_knowledge() {
  ...
  case "$topic" in
    overview)
      cat <<'EOF'
TinkerOS is a hardware-throughput desktop OS layered on a real Linux kernel — code
inside the Linux code, not a separate layer. It is designed for people who want a
powerful, beautiful, and helpful desktop experience. TinkerOS comes with three
isolated "worlds" that transform the entire OS for different purposes, and ships
with TinkerAI, a built-in agentic AI assistant accessible from any world.
...
- TinkerAI: built-in AI assistant, pop it with Ctrl+Alt+Gr from any world
...
- Ships as a bootable ISO (~7.6 GB) — TinkerOS-v1.3.iso
- Dual Source License v1.0 (Option A public / Option B private, no fee)
EOF
      ;;
    worlds)
      cat <<'EOF'
TinkerOS has THREE isolated worlds — each one transforms the entire desktop:
...
Every world supports Ctrl+Alt+Gr to pop TinkerAI.
EOF
      ;;
    features)
      cat <<'EOF'
...
AI & ASSISTANT:
- TinkerAI: built-in agentic AI assistant. Ask anything, control the computer, get help.
  Pop from any world: Ctrl+Alt+Gr
  CLI: tinker-ai ask "question", tinker-ai chat "message"
...
- GDM greeter themed for TinkerOS
...
AI ASSISTANT:
  Ctrl+Alt+Gr                    Pop TinkerAI from any world
  tinker-ai ask "question"       Ask TinkerAI anything
  tinker-ai chat "message"       Chat with TinkerAI
  tinker-ai screenshot           Take a screenshot (AI reads it)
  tinker-ai todo add "task"      Add a todo item
  tinker-ai remind "msg" 30m     Set a reminder
  tinker-ai contacts             List saved contacts
  tinker-ai wallet               Check digital wallet
  tinker-ai open firefox         Open an application
  tinker-ai history              View conversation history
  tinker-ai clear-history        Clear chat history
...
SYSTEM:
  tinkeros-control-center        Open the Control Center (224+ tools)
...
EOF
    ...
    ui-layout)
      cat <<'EOF'
TinkerOS Desktop Layout:

TOP BAR (Panel):
- Left: Application menu (TinkerOS logo)
- Center: Clock and date
- Right: System tray (WiFi, Bluetooth, Volume, Battery, User menu)

APPLICATION MENU (click the TinkerOS logo):
...
- Tab completion for TinkerOS commands
...
- Right-click context menu with TinkerOS tools
...
EOF
    ...
    troubleshooting)
      cat <<'EOF'
Common TinkerOS Issues & Solutions:
...
AI Engine: TinkerAI (Ctrl+Alt+Gr)
ISO Size: ~7.6 GB
License: Dual Source License v1.0 (Option A public / Option B private)
Download: https://sourceforge.net/projects/tinkeros/files/v1.3/TinkerOS-v1.3.iso/
EOF
      ;;
...
tk_random_fact() {
  local facts=(
    "TinkerOS has three isolated worlds — HACK for security, NORMAL for everyday use, GAME for maximum gaming performance."
    "Switch worlds anytime with Space+Shift+1/2/3 or Ctrl+Arrow keys."
    "TinkerAI pops up in any world with Ctrl+Alt+Gr — it can take screenshots and read what's on your screen."
    "The Searchie engine indexes your files so you can find anything instantly with Tab+F7."
    "TinkerOS has 224+ built-in system tools in the Control Center and 45 territory tools."
    ...
    "TinkerOS uses glassmorphism design — frosted glass with blur effects and 18px rounded corners."
    "You can ask TinkerAI to set reminders, manage contacts, or track your budget."
    "The dust dislodger spins fans at special intervals to clean dust — but you must calibrate first."
    "TinkerOS is built on a real Linux kernel with 27 custom C modules wired into the real scheduler."
    ...
    "TinkerOS ships as a bootable ISO with branded Plymouth splash and GRUB menu."
