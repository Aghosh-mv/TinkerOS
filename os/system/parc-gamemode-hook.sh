#!/bin/bash
# KorrinOS kernel GameMode hook — bridges Feral GameMode activation to the
...
#
# Requires the KorrinOS kernel (CONFIG_TINKER_GAMEMODE=y). If the proc node
...
do_status() {
    if [ -r "$PROC_GAMEMODE" ]; then
        ...
        echo "kernel gamemode proc node not present (non-KorrinOS kernel?)"
    fi
}
