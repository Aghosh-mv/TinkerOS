#!/bin/bash
# tinker-dust — KorrinOS fan dust-dislodger CLI (kernel: /proc/tinker/dust_dislodger).
#
...
cmd() { echo "$@" > "$PROC" 2>/dev/null || { echo "tinker-dust: write to $PROC failed (KorrinOS kernel needed)" >&2; exit 1; }; }
...
case "${1:-}" in
  ""|-h|--help) usage ;;
  status)   cat "$PROC" 2>/dev/null || echo "no $PROC (KorrinOS kernel required)"; echo ;;
