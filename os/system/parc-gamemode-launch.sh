#!/bin/bash
# tinker-gamemode — launch an app with KorrinOS performance mode.
#
# Two-step boost:
#   1. Feral GameMode (gamemoderun) — userspace governor/GPU/graphics tuning.
#   2. KorrinOS in-kernel gamemode   — schedutil puts a P-state headroom step on
...
# battery-friendly schedule.  Degrades gracefully when the KorrinOS kernel or
# Feral GameMode is absent.

set -u

HOOK="${TINKER_HOOK:-/opt/korrinos/os/system/tinker-gamemode-hook.sh}"
