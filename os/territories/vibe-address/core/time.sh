...
ve_time_between() {  # phrase
  ...
  if [[ "$phrase" =~ between[[:space:]]+(.+)[[:space:]]+and[[:space:]]+(.+) ]] ; then
    ...
    IFS=' ' read -r aa x <<<"$a"; IFS=' ' read -r bb x <<<"$b"
...
ve_time_chomp() {  # phrase, fallback_lo, fallback_hi -> lo hi (echo)
  ...
  local lo hi
  lo=$(ve_time_era      "$phrase") && { IFS=' ' read -r lo hi <<<"$lo"; printf '%s %s' "$lo" "$hi"; return 0; }
  lo=$(ve_time_week_of_month "$phrase") && { IFS=' ' read -r lo hi <<<"$lo"; printf '%s %s' "$lo" "$hi"; return 0; }
  lo=$(ve_time_weekday "$phrase") && { IFS=' ' read -r lo hi <<<"$lo"
      # narrow by time-of-day if given
      local h; h=$(ve_time_hod "$phrase") && { IFS=' ' read -r dh _ <<<"$h";
        local daylo; daylo=$(ve_time_day_start "$lo")
        lo=$(( daylo + (dh - daylo) )); hi=$(( lo + 3600 )); }
      printf '%s %s' "$lo" "$hi"; return 0; }
  lo=$(ve_time_hod    "$phrase") && { IFS=' ' read -r lo hi <<<"$lo"; printf '%s %s' "$lo" "$hi"; return 0; }
  lo=$(ve_time_between "$phrase") && { IFS=' ' read -r lo hi <<<"$lo"; printf '%s %s' "$lo" "$hi"; return 0; }
  printf '%s %s' "$flo" "$fhi"
  return 0
}
