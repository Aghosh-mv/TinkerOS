...
ve_index_fp_insert() {
  local fp="$1" path="$2" vtype="$3" catpath="$4" epoch="$5" meta="$6"
...
ve_index_tokens_to_fps() {
  ...
  # intersect: fps appearing for all tokens (consensus)
  sort "$tmp" | uniq -c | sort -rn | \
    awk -v n=$(wc -l < <(echo "$tokens" | sed '/^$/d' | wc -l) ) \
        '$1 >= max(1, n/2) {print $2}'   # at least 50% token agreement
  rm -f "$tmp"
}
...
ve_index_fp_to_envelope() {
  local fp="$1" fpd="$VIBE_INDEX/fp/$fp"
  [ -f "$fpd" ] && tail -1 "$fpd"
}
