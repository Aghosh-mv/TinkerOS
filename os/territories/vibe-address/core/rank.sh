...
ve_rank_format_result() {
  # stdin: "score|cat1|fp|epoch|type|path|catpath|meta" (one line)
  local rank=0
  while IFS='|' read -r score cat1 fp epoch etype epath ecat emeta; do
    rank=$((rank + 1))
    local ago; ago=$(ve_rank_relative_time "$epoch")
    printf '  %2d  [%3d%%] %s (%s)  %s ago\n' \
