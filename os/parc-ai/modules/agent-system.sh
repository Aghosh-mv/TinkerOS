...
# Execute a terminal command (narrates what it's doing)
agent_exec() {
  local cmd="$1" timeout="${2:-30}"
  
  # Narrate
  ai_narrate "Running: $cmd" 2000 2>/dev/null
  ...
  if [ $rc -eq 0 ]; then
    ai_narrate "Done!" 1500 2>/dev/null
    echo "$output"
  else
    ai_narrate "That didn't work. Let me see what went wrong..." 2000 2>/dev/null
    echo "I tried running that but hit a snag. Here's what happened: $output"
  fi
  return $rc
}
...
# Open any application by name (narrates what it's doing)
agent_open_app() {
  local app="$1"
  local logfile="$AGENT_DIR/logs/apps.log"
  
  # Narrate
  ai_narrate "Opening $app..." 2000 2>/dev/null
  ...
  case "${app,,}" in
    *browser*|*firefox*|*chrome*|*chromium*|*brave*)
      for b in brave-browser vivaldi chromium-browser google-chrome firefox; do
        if command -v "$b" &>/dev/null; then
          ...
          ai_narrate "Opened $b for you" 1500 2>/dev/null
    ...
    *editor*|*code*|*vscode*)
      for e in code codium vim nano emacs; do
        if command -v "$e" &>/dev/null; then
          nohup "$e" &>/dev/null &
          ai_narrate "Opened $e" 1500 2>/dev/null
    ...
    *files*|*finder*|*nautilus*|*dolphin*)
      for f in nautilus dolphin thunar pcmanfm nemo; do
        if command -v "$f" &>/dev/null; then
          nohup "$f" &>/dev/null &
          ai_narrate "Opened file manager" 1500 2>/dev/null
    ...
    *settings*|*preferences*)
      ...
      ai_narrate "Opened settings" 1500 2>/dev/null
    ...
    *spotify*|*music*)
      for m in spotify rhythmbox vlc Clementine; do
        if command -v "$m" &>/dev/null; then
          nohup "$m" &>/dev/null &
          ai_narrate "Opened $m" 1500 2>/dev/null
  ...
  if command -v "$app" &>/dev/null; then
    nohup "$app" &>/dev/null &
    ai_narrate "Opened $app" 1500 2>/dev/null
    echo "Opened $app"; return 0
  fi
  
  ai_narrate "I couldn't find $app on your system" 2000 2>/dev/null
  echo "I couldn't find $app. Is it installed?"
  return 1
}
