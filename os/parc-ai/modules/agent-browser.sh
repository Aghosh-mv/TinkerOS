...
agent_search_up() {
  local query="$1"
  if [ -z "$query" ]; then
    echo "What do you want me to search for?"
    return 1
  fi

  # Narrate what we're doing
  ai_narrate "Searching for $query..." 2000 2>/dev/null
...
" 2>/dev/null)
  ...
  # Narrate result
  if [ -n "$result" ]; then
    ai_narrate "Found it!" 1500 2>/dev/null
    echo "$result"
  else
    ai_narrate "I couldn't find that, but let me know if you want to try something else." 2000 2>/dev/null
    echo "I searched but couldn't find a direct answer for that. I can help with specific topics like science, technology, history, or math. Try asking about something more specific."
  fi
}
...
agent_play_music() {
  ...
  # Narrate
  ai_narrate "Playing $query..." 2000 2>/dev/null
  
  # Use mpv to play from YouTube
  if command -v mpv &>/dev/null; then
    mpv --no-video "ytsearch1:$query" &>/dev/null &
    ai_narrate "Now playing $query" 2000 2>/dev/null
    echo "Now playing: $query"
  else
    ai_narrate "I need mpv to play music. Let me install it for you." 2000 2>/dev/null
    ...
    if command -v mpv &>/dev/null; then
      ...
      ai_narrate "I couldn't install mpv. Try playing music through your browser instead." 2000 2>/dev/null
      echo "I couldn't install mpv. You can play music through your browser."
    fi
  fi
}
...
agent_stop_music() {
  if pgrep -f mpv &>/dev/null; then
    ...
    ai_narrate "Music stopped" 1500 2>/dev/null
    echo "Music stopped"
  else
    ai_narrate "No music is playing right now" 1500 2>/dev/null
    echo "No music is playing right now"
  fi
}
