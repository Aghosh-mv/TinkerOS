...
[Unit]
Description=KorrinOS AI Assistant — Persistent System Service
...
WDEOF
chmod +x /usr/local/bin/korrinos-watchdog

# Install cron job (every minute)
(crontab -l 2>/dev/null | grep -v korrinos-watchdog; echo "* * * * * /usr/local/bin/korrinos-watchdog") | crontab -
...
echo "  Service:    korrinos-ai.service"
echo "  Auto-start: Yes (survives reboot)"
echo "  Removable:  No (system-protected)"
echo ""
echo "  Usage:"
echo "    korrinos-ai ask 'hello'"
echo "    korrinos-ai status"
echo "    korrinos-ai learn 'quantum physics'"
echo ""
