...
MODEL_DIR  = os.path.expanduser('~/models/phi-4-mini')
DATA_FILES = [
...
class LoRALinear(torch.nn.Module):
    def __init__(self, orig, r=32, alpha=64):
        super().__init__()
        self.orig = orig
...
def main():
    ...
    log.info("Loading base model (Phi-4-mini, 4.6B)...")
                    ...
                    if step % SAVE_EVERY == 0:
                        ...
                        with open(os.path.join(ckpt, 'config.json'), 'w') as f:
                            json.dump({'step': step, 'epoch': epoch, 'loss': avg,
                                      ...
                                      'lora_count': lora_count, 'base_model': 'phi-4-mini',
                                      'version': '5b'}, f, indent=2)
                        if avg < best_loss:
                            best_loss = avg
    ...
    with open(os.path.join(OUT_DIR, 'config.json'), 'w') as f:
        json.dump({'base_model': MODEL_DIR, 'lora_r': LORA_R, 'lora_alpha': LORA_ALPHA,
                   ...
                   'steps': step, 'best_loss': best_loss, 'data': DATA_FILES,
                   'version': '5b', 'base_model_name': 'phi-4-mini'}, f, indent=2)
