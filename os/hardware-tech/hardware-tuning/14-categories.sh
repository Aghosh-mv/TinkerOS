#!/bin/bash
# KorrinOS Hardware Tuning - All 14 Categories
...
show_all(){ echo "=== KorrinOS 14-Category Hardware Tuning ==="; for f in "$MASTER_DIR"/*.sh; do [ "$f" = "$MASTER_DIR/14-categories.sh" ] && continue; echo "  $(basename $f .sh)"; done; }
case "${1:-help}" in
  list|help) show_all ;;
  *) show_all ;;
esac
