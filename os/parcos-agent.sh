#!/bin/bash
# KorrinOS Agentic AI launcher — the preinstalled assistant.
...
#
#   pop    : open a small gnome-terminal running opencode in KorrinOS tree
...
pop() {
  echo "Opening KorrinOS Agentic AI (opencode)..."
  # tiny window: ~90 cols x 26 rows, single pane, black bg for sir-like focus
  gnome-terminal --title "KorrinOS Agent" --geometry 100x28 \
    -- bash -c "cd '$WORK_TREE'; '$OC'" 2>/dev/null \
    || xterm -title "KorrinOS Agent" -geometry 100x28 -e bash -c "cd '$WORK_TREE'; exec '$OC'" 2>/dev/null \
...
EOF
    ...
    if command -v gsettings >/dev/null 2>&1 && gsettings get org.gnome.settings-daemon.plugins.media-keys custom-keybindings >/dev/null 2>&1; then
      ...
      gsettings set org.gnome.settings-daemon.plugins.media-keys custom-keybindings "['/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/korrinos-agent/']" 2>/dev/null || true
      gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/korrinos-agent/ name "KorrinOS Agent" 2>/dev/null || true
      gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/korrinos-agent/ command "$SELF pop" 2>/dev/null || true
      gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/korrinos-agent/ binding "<Control><Alt>ISO_Level3_Shift" 2>/dev/null || true
...
uninstall_bind() {
  ...
  sed -i '/# KorrinOS Agent : Ctrl+AltGr/,+1d' "$HOME/.xbindkeysrc" 2>/dev/null || true
